...
- **Preconditions:** The visitor does not have an existing account.
- **Postconditions:** A `User` record exists with a hashed password, and a verification message has been sent.

### Main Success Scenario

1. The visitor submits an email, display name, and password.
2. The system validates the email format and password strength.
3. The system confirms no account already exists for that email.
4. The system stores the user with a hashed password and `isVerified` false.
5. The system sends a verification message to the submitted email.

### Extensions

- **3a.** An account already exists for that email: the system reports success generically and sends a notification to the existing address, rather than revealing that the email is registered (avoids account enumeration).
- **2a.** Password does not meet strength requirements: the system rejects the registration and states the requirement that failed.
